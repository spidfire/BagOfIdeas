...
use \Exception;
use \PDO;
use BagOfIdeas\Models\User\Wiki as ChildWiki;
use BagOfIdeas\Models\User\WikiQuery as ChildWikiQuery;
use BagOfIdeas\Models\User\Map\WikiTableMap;
...
abstract class Wiki implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\BagOfIdeas\\Models\\User\\Map\\WikiTableMap';
    ...
    /**
     * Initializes internal state of BagOfIdeas\Models\User\Base\Wiki object.
     */
     ...
     * @return $this|\BagOfIdeas\Models\User\Wiki The current object (for fluent API support)
     */
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[WikiTableMap::COL_NAME] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : WikiTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : WikiTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->created_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : WikiTableMap::translateFieldName('UpdatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            ...
            return $startcol + 4; // 4 = WikiTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\BagOfIdeas\\Models\\User\\Wiki'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(WikiTableMap::COL_NAME)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\BagOfIdeas\Models\User\Wiki
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\BagOfIdeas\Models\User\Wiki The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(WikiTableMap::COL_NAME)) {
            $criteria->add(WikiTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(WikiTableMap::COL_CREATED_AT)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \BagOfIdeas\Models\User\Wiki (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \BagOfIdeas\Models\User\Wiki Clone of current object.
     * @throws PropelException
     */
